...
Next to `.opossum` files, OpossumUI supports the following formats:

- ScanCode JSON
- OSS Review Toolkit JSON/YAML
- more to come

OpossumUI can export:

- SPDX-2.2 (Lite) JSON/YAML
- CSV attributions lists
- CSV follow-up lists
...
Legacy opossum files (`.json`/`.json.gz`) and output files produced by some supported scanners can be imported from
within OpossumUI. Currently, the following scanner output file types can be directly imported:
