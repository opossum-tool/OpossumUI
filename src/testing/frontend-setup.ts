...
import type { setDatabaseInitialized as setDatabaseInitializedType } from '../Frontend/util/backendClient';
import { DEFAULT_USER_SETTINGS } from '../shared/shared-constants';
import type { ElectronAPI } from '../shared/shared-types';
import { unexpectedBackendCall } from './backend-integration';
...
global.window.electronAPI = {
  ...
  api: vi.fn().mockImplementation(unexpectedBackendCall),
...
vi.spyOn(Element.prototype, 'scrollIntoView').mockImplementation(() => void 0);
vi.spyOn(Range.prototype, 'getBoundingClientRect').mockReturnValue(mockDOMRect);
...
beforeEach(async () => {
  if (typeof window.electronAPI?.api === 'function') {
    vi.spyOn(window.electronAPI, 'api')
      .mockReset()
      .mockImplementation(unexpectedBackendCall);
  }
  const { setDatabaseInitialized } = await vi.importActual<{
    setDatabaseInitialized: typeof setDatabaseInitializedType;
  }>('../Frontend/util/backendClient');
