...
describe('FileConverter executable resolution', () => {
  ...
  it('falls back to the packaged non-suffixed CLI on Windows when no .exe exists', () => {
    electronMock.app.isPackaged = true;
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32',
    ...
    vi.spyOn(fs, 'existsSync').mockImplementation((candidate) => {
      return candidate === path.join('/resources', 'bin', 'opossum-file-cli');
    });
  ...
  it('uses the packaged .exe CLI on Windows when it exists', () => {
    electronMock.app.isPackaged = true;
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32',
    ...
    vi.spyOn(fs, 'existsSync').mockImplementation((candidate) => {
      return (
        candidate === path.join('/resources', 'bin', 'opossum-file-cli.exe')
      );
    });
  ...
  it('falls back to a repo-level bin directory in development', () => {
    ...
    vi.spyOn(fs, 'existsSync').mockImplementation((candidate) => {
      return candidate === path.join('/repo', 'bin', 'opossum-file-cli');
    });
