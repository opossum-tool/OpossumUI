...
export const text = {
  ...
  auditingOptions: {
    ...
    confidence: 'Confidence',
    confidenceOf: (value: number) => `confidence of ${value}`,
  ...
  diffPopup: {
    ariaLabel: 'diff popup',
    attribution: 'Attribution',
    attributionType: 'Attribution type',
    comparisonSides: 'Comparison sides',
    comment: 'Comment',
    copyright: 'Copyright',
    derivedPurl: 'Derived PURL',
    emptyPurl: '—',
    ...
    copyLeftToRight: 'Copy left value to right',
    copyRightToLeft: 'Copy right value to left',
    emptyField: 'Empty',
    readOnly: 'Read-only',
    signal: 'Signal',
    unsavedChanges: 'Unsaved changes',
    undoAuditingChanges: (label: string) => `Undo auditing changes on ${label}`,
    undoField: (field: string, label: string) => `Undo ${field} on ${label}`,
    restoreField: (value: string, label: string) =>
      `Restore ${value} on ${label}`,
  },
  errorBoundary: {
    outdatedAppVersion:
