...
describe('useComparisonState', () => {
  ...
  it('keeps zero confidence as a mutation candidate', () => {
    const { result } = renderHook(() =>
      useComparisonState(
        item({ isExternal: true }),
        item({ packageInfo: packageInfo({ id: 'right' }) }),
        false,
      ),
    );
  ...
  it('preserves opening snapshots when comparison items refresh', () => {
