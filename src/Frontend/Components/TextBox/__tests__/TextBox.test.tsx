...
import { renderWithTheme } from '../../../test-helpers/render';
import { TextBox } from '../TextBox';

describe('TextBox', () => {
  it('renders text and label', () => {
    renderWithTheme(<TextBox title={'Test Title'} text={'Test Content'} />);
  ...
  it('forwards the readOnly state to the input', () => {
    renderWithTheme(
      <TextBox title={'Test Title'} text={'Test Content'} readOnly />,
    );
  ...
  it('preserves multiline focus and selection when the end icon changes', async () => {
    ...
    const { rerender } = renderWithTheme(<TextBox {...props} />);
    const input = screen.getByRole('textbox');
